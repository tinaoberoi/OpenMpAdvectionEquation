...
double calc_first_order_value(int CHUNK_SIZE, int i, int j, double m1[CHUNK_SIZE + 2][CHUNK_SIZE + 2], double u, double v, double dt, double dx);
...
int main(int argc, char *argv[])
{
    ...
    if (taskid == 0)
    {
        ...
        printf("Time taken for first order uniform: %lf \n", end-start);
    }
    else
    {
            ...
            double right_recv_arr[CHUNK_SIZE], double dt, double dx, double u, double v, int taskid)
{
    ...
    for (int i = 1; i < CHUNK_SIZE+1 ; i++)
    {
        ...
        {
            m2[i][j] = calc_first_order_value(CHUNK_SIZE, i, j, m1, u, v, dt, dx);
        }
    }
}
